...
use crate::ServiceDiscoverabilityClient;
use anyhow::Result;
use cloud_util::Api;
use reqwest::Method;

pub struct AssignmentsClient<'a> {
    root: &'a ServiceDiscoverabilityClient,
}

impl<'a> AssignmentsClient<'a> {
    pub(crate) fn new(client: &'a ServiceDiscoverabilityClient) -> Self {
        Self { root: client }
    }
    
    pub async fn get(&self, request: GetAssignmentRequest) -> Result<Assignment> {
        let url_suffix = format!("service-discoverability/{}", request.instance_id);
